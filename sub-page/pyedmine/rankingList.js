...
function initSceneSelector(data) {
  ...
  const uniqueScenes = [...new Set(scenes.map(scene => parseScene(scene).scene))];

  const html = uniqueScenes.map(scene => `
...
function initMetricSelector(sceneName) {
  ...
  const html = Array.from(metrics).map(metric => `
...
function renderTable() {
  ...
  thead += `<th class="sota-column">SOTA (w | t)</th></tr>`;

  // 构建数据行
  const tbody = Array.from(models).map(model => {
      let row = `<td>${model}</td>`;
      datasetNames.forEach(dataset => {
      ...
      // 计算 SOTA
      const sota = calculateSOTA(model, datasets, metric);
      row += `<td class="sota-column">${sota.win} | ${sota.tie}</td>`;
      return `<tr>${row}</tr>`;
  }).join('');
...
// 工具函数：计算 SOTA
function calculateSOTA(model, datasets, metric) {
  let win = 0, tie = 0;
  const isMinBetter = ['RMSE', 'MAE'].includes(metric);
  ...
  Object.values(datasets).forEach(dataset => {
      ...
      if (modelValue === bestValue) {
          const isTie = sorted.filter(entry => entry[1] === bestValue).length > 1;
          if (isTie) tie++;
          else win++;
      } else if (Math.abs(modelValue - bestValue) < 0.001) {
          tie++;
      }
  });
